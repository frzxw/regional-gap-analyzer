...
def extract_tahun(tahun):
    for year in tahun:
        try:
            ...
            persentase_kemiskinan_df = pd.read_csv(f'../../../raw/persentase-penduduk-miskin/{year}.csv', skiprows=4)
            count = 0

            for index, row in persentase_kemiskinan_df.iterrows():
...
collection = utils.get_mongo_collection("persentase_penduduk_miskin")
extract_tahun(tahun)